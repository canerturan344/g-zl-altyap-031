...
const kadın = message.guild.roles.find(x => x.name === "")
...
let verildi = üye.addRole(kadın) && üye.removeRole(kayıtsız) && üye.setNickname(isim);
if(verildi){
    message.delete()
let s = new RichEmbed()
.setTitle("Başarılı")
.setDescription(`${üye} Adlı kullanıcıya ${kadın} rolünü verdim`)
.setTimestamp()
.setColor(ayarlar.renk)
.setFooter(`Komutu Kullanan Yetkili ` + message.author.tag)
message.channel.send(s).then(x => x.delete(5000));
